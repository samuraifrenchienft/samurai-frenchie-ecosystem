# $CHIE'S — Tokenomics (Solana Meme Coin)

This document summarizes the tokenomics for **$CHIE's**, a Solana-based meme coin
focused on degen energy, community raids, and fun. No complex over-engineered
mechanics — just a clean launch with transparent numbers and room to grow.

---

## 1. Core Facts

- **Chain:** Solana  
- **Standard:** SPL  
- **Ticker:** $CHIE'S  
- **Total Supply:** 420,760,000 tokens  
- **Decimals:** 9 (standard SPL)  
- **Buy/Sell Tax:** 0% (no hidden taxes)  
- **Mint Authority:** Revoked after deployment (no new tokens can be minted)  
- **Freeze / Blacklist:** None planned (no stealth restrictions on holders)  

---

## 2. Allocation Overview

**Total Supply:** 420,760,000 $CHIE'S (100%)

### 2.1 Liquidity & Fair Launch — 83%
**349,230,800 tokens**

- Added to DEX liquidity and circulating at launch  
- Majority of supply goes here to keep launch as fair as possible  
- LP planned to be locked or burned so the community can trade without rug-pull risk  

### 2.2 Community / Quests / Airdrops — 6%
**25,245,600 tokens**

- For community quests, raids, early supporters, and social campaigns  
- Portion available at launch for activity and hype  
- Remaining amount streamed over time to reward ongoing engagement  

### 2.3 Promotion — 5%
**21,038,000 tokens**

- Promos, partnerships, KOL deals  
- DEX boosts and strategic visibility pushes  
- Used transparently with clear announcements around any major spends  

### 2.4 Team & Ops — 3%
**12,622,800 tokens**

- Core contributors, design, development support  
- Ongoing operational costs for maintaining and improving the project  
- Subject to a longer Lock to align team incentives with long-term health  

### 2.5 Future Investors & Advisors — 3%
**12,622,800 tokens**

- Reserved for potential strategic angels, degen funds, or advisors  
- Any deals from this pool will use short, transparent vesting schedules  
- If unused, this pool can be burned or redirected to community-focused uses  

---

## 3. Coin Launch Philosophy

The goal is to keep $CHIE'S as close to a **fair meme launch** as possible,
while still leaving room for growth, support, and strategic partners.

- Liquidity & Fair Launch allocation dominates the supply (83%).  
- Non-LP allocations unlock more slowly or under clear, announced conditions.  

**Example vesting approach:**

- **Community & Promotion:**  
  - Partial unlock at launch for initial campaigns  
  - Remaining tokens sunlocked **6 months**

- **Team & Ops:**  
  - Long lock with 50% **12 month**  
  -  Rest after **18 months**   

- **Future Investors & Advisors:**  
  - Short degen-style vesting (for example, **35–50% at TGE** and the rest unlocked over **1–2 months**)  
  - Only used if strategic deals make sense for the project  

---

## 4. Launch & Liquidity Plan (Summary)

- Launch on a Solana DEX (e.g., Raydium)  
- Majority of supply added to liquidity at launch  
- LP to be **locked or burned** to build trust  
- No taxes, no confusing mechanics — just trading, memes, and community  

---

_Last updated: 2025-11-24_
